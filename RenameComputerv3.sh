...
ComputerName="$(/usr/bin/osascript -e 'tell application "System Events" to display dialog "Please enter the computer name:" default answer "" with title "Computer Name" with text buttons {"OK"} default button 1 with hidden answer with icon file "'"${LOGO_ICNS//\"/\\\"}"'"' -e 'text returned of result')"
